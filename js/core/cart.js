...
import { isCommerciallyPurchasable, confirmedPrice, knownStock } from './commercial.js';
export const MAX_QUANTITY = 99;
export function emptyCart(scope) { scopeKey(scope, 'cart'); return { ...scope, version: 1, lines: [] }; }
...
export function quoteCart(cart, business, products, fulfillment = 'pickup') {
  ...
  const deliveryFee = fulfillment === 'delivery' ? business.deliveryFee : 0;
  ...
  requireValue(fulfillment !== 'delivery' || subtotal >= business.minimumOrder, 'MINIMUM_ORDER', `El pedido mínimo para envío es $${business.minimumOrder}.`);
