...
with sync_playwright() as p:
    ...
    try:
        ...
        expect(page.locator('#taxi-origin-input')).to_have_value('Plaza San Martín')
        ...
        expect(page.locator('.taxi-driver-card')).to_contain_text('Conductor demo')
        expect(page.locator('.taxi-driver-card')).to_contain_text('Móvil DEMO')
        expect(page.locator('.taxi-driver-card')).not_to_contain_text('Carlos Morales')
        expect(page.locator('.taxi-driver-card')).not_to_contain_text('4.9')
        trip_url = page.url
        page.screenshot(path=str(EVIDENCE / 'taxi-tracking-desktop.png'), full_page=True)
        record('TRACKING de taxi con mapa esquemático y móvil demo asignado', 'PASS')

        # D. Panel del Chofer (#taxi-driver)
        navigate(page, 'taxi-driver')
        expect(page.get_by_role('heading', name='Móvil DEMO · Conductor demo')).to_be_visible()
