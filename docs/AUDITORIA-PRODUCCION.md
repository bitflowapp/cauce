...
| N-20 | P1 | El primer backup real (run 36075767563) no podía restaurarse: Supabase en la nube va por delante de la CLI (`auth.mfa_recovery_code_sets`, `storage.buckets.lifecycle_configuration`), la restauración usaba siempre las 9 migraciones aunque producción tuviera 8, y la copia cifrada se hacía recién después de la prueba (si fallaba, no quedaba copia). | Resuelto junto con los commits de Marco (86832df, 7f9fa92, b0c6b44): el volcado se adapta al stack de prueba e informa lo omitido (`scripts/lib/volcado.mjs`, con pruebas; en `public`/`private` cualquier diferencia falla), la restauración usa las migraciones del origen y aplica las pendientes sobre la copia, y la copia cifrada va primero. |
| N-21 | P2 | La comparación de esquema del backup marcaba como desvío privilegios que fija Supabase al crear el proyecto: `GRANT ALL ... TO service_role` (53 líneas) y los privilegios por defecto de `anon`/`authenticated`/`service_role` (15), que cambiaron entre versiones de la plataforma. | Resuelto: se informan aparte; los por defecto de `anon`/`authenticated` sólo mientras falte 20260924120000, que los revoca (después tienen que coincidir). Cualquier otro privilegio distinto sigue siendo desvío. |
| N-22 | P2 | `configure-auth.mjs` dejaba contraseña, rotación de tokens y vencimiento de enlaces en los valores por defecto del proyecto aunque el paso `auth` los verificaba, y mandaba `smtp_port` como número (la API lo declara texto). | Resuelto: aplica lo mismo que `supabase/config.toml` (10+ con letras y números, rotación, enlaces de 1 h, reautenticación para cambiar la contraseña); nombres y tipos verificados contra la especificación pública de la API. |
| N-23 | P2 | Los registros del proyecto real (24 h) mostraron `invalid column for filter business_id`: una visita sin ninguna sesión en `#panel/<id>` o `#pedido/<id>` igual abría un canal de Realtime sobre pedidos, que la base rechaza desde 20260924120000; quedaba como error en el registro de administración y consultaba cada 30 s. En `#pedido/<id>` además veía "Tu cuenta no tiene permiso" con un Reintentar que nunca funciona (caso real: el enlace abierto en otro navegador), y "Mis pedidos" le decía "Compraste sin cuenta" a quien nunca compró. | Resuelto: sin sesión no hay canal ni sondeo, y el pedido explica cómo verlo (enlace de seguimiento o ingresar). E2E: ninguna suscripción de la visita y el aviso visible; falla con el código anterior. |
| N-24 | P2 | Supabase retiró `analytics/endpoints/logs.all` el 23/09 (410) y las tablas por servicio: la revisión de registros por la API dejó de funcionar. | Resuelto: paso `registros` sobre la tabla unificada `logs` (SQL de ClickHouse); si la consulta de 5xx no corre, el paso falla; la salida enmascara correos e IP. |
| N-25 | P2 | El recorrido completo con navegador contra el proyecto real sólo podía correr después de publicar (`smoke-publicado`), y los smokes (que crean y borran datos QA en producción) no pedían confirmación. | Resuelto: `smoke-previo` corre el mismo smoke con el build de producción servido en el runner, antes de publicar; los dos smokes exigen `aplicar` y `confirmar`. |
| N-26 | P2 | `aria-busy="false"` (la señal de vista estable que usan las tecnologías de asistencia y todas las pruebas de navegador) ya valía mientras una acción esperaba al servidor o una navegación estaba pedida, y al ingresar con un solo comercio la redirección a su panel podía pisar la vista que la persona ya había elegido. En CI hizo fallar 1 de cada ~7 corridas en WebKit (un toque que caía en pleno redibujo se perdía). | Resuelto: la vista está ocupada mientras haya una acción esperando al servidor (no mientras un diálogo espera a la persona), una navegación pendiente o un dibujo en curso, y la redirección sólo ocurre si la persona sigue en la lista. E2E determinista con respuestas demoradas: falla en Chromium y WebKit con el código anterior; 10 vueltas seguidas del archivo de recorridos sin fallos (antes 3 de 14). |
